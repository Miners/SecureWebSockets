...
public class WebSocketWriter extends Handler {
	 ...
	 */
	public WebSocketWriter(final Looper looper, final Handler master, final Socket socket, final WebSocketOptions options) {
		super(looper);
...
//		this.mOutputStream = outputStream;
	...
	private void sendClientHandshake(final WebSocketMessage.ClientHandshake message) throws IOException {
		...
		final String query = message.getURI().getQuery();
	...
	private void sendClose(final WebSocketMessage.Close message) throws IOException, WebSocketException {
		if (message.getCode() > 0) {
			...
			if (message.getReason() != null && !(message.getReason().length() > 0)) {
				final byte[] pReason = message.getReason().getBytes(WebSocket.UTF8_ENCODING);
	...
	private void sendTextMessage(final WebSocketMessage.TextMessage message) throws IOException, WebSocketException {
		final byte[] payload = message.mPayload.getBytes(WebSocket.UTF8_ENCODING);
	 ...
	 * Send WebSockets binary message.
	 */
	private void sendRawTextMessage(final WebSocketMessage.RawTextMessage message) throws IOException, WebSocketException {
		if (message.mPayload.length > mWebSocketOptions.getMaxMessagePayloadSize()) {
			throw new WebSocketException("message payload exceeds payload limit");
		}
	 ...
	 */
	protected void sendFrame(final int opcode, final boolean fin, final byte[] payload, final int offset, final int length) throws IOException {
		// first octet
		byte b0 = 0;
		if (fin) {
			b0 |= (byte) (1 << 7);
